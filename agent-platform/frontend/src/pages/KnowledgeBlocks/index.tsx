...
export default function KnowledgePage({ scope = "agent" }: { scope?: "agent" | "library" }) {
  ...
  const resources = useAgentResourceLibrary<Block>(
    agentId,
    "knowledge-blocks",
    "/knowledge-blocks/",
  );
  ...
  const toggle = async (block: Block) => {
    ...
    setError("");
    try {
      await api(`/knowledge-blocks/${block.key}`, {
        method: "PATCH",
        body: JSON.stringify({ is_enabled: !block.is_enabled }),
      });
      await resources.refresh();
    } catch (value) {
      setError(value instanceof Error ? value.message : "No se pudo actualizar el bloque.");
    } finally {
      setToggling(null);
    }
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      setError(value instanceof Error ? value.message : "No se pudo guardar el bloque.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const createBlock = async () => {
    ...
    setError("");
    try {
      const created = await api<Block>("/knowledge-blocks/", {
        method: "POST",
        body: JSON.stringify({
          key: newKey.trim(),
          title: newTitle.trim(),
      ...
      if (agentId)
        await api(`/agents/${agentId}/knowledge-blocks/${created.id}`, { method: "PUT" });
      setShowCreate(false);
      setNewKey("");
      setNewTitle("");
      setNewContent("");
      ...
      setError(value instanceof Error ? value.message : "No se pudo crear el bloque.");
    } finally {
      setCreating(false);
    }
  };
    ...
    return (
      <div className="max-w-4xl">
        <header className="mb-6 flex flex-wrap items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-semibold">Editar recurso compartido</h2>
            <code className="text-xs text-[var(--accent)]">{editing.key}</code>
          </div>
          {canEdit && (
            <button
              type="button"
              onClick={save}
              disabled={saving}
              className="ml-auto flex items-center gap-2 rounded bg-[var(--accent)] px-4 py-2 text-sm text-white disabled:opacity-50"
            >
        ...
        <section className="space-y-4 rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-5">
          ...
          <p className="text-xs text-amber-300">
            Este bloque pertenece a la biblioteca compartida. Editarlo afecta a todos los agentes
  ...
  const BlockCard = ({
    ...
    <article
      className={`rounded-lg border bg-[var(--bg-card)] p-4 ${block.is_enabled ? "border-[var(--border-color)]" : "border-[var(--border-color)] opacity-60"}`}
    >
      <div className="flex items-start gap-3">
        <div className="min-w-0 flex-1">
          <h4 className="font-semibold">{block.title}</h4>
          <code className="text-xs text-[var(--text-muted)]">{block.key}</code>
        </div>
        <span className="rounded bg-[var(--bg-hover)] px-2 py-1 text-xs">
          orden {block.sort_order}
        </span>
      </div>
      <p className="mt-3 line-clamp-3 whitespace-pre-wrap text-sm text-[var(--text-secondary)]">
        {block.content}
      </p>
      {canEdit && (
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          >
            {block.is_enabled ? "Deshabilitar globalmente" : "Habilitar globalmente"}
          </button>
          <button
            type="button"
          ...
          {assignment === "assigned" && (
            <button
              type="button"
              onClick={() => resources.unassign(block.id)}
              disabled={resources.busyId === block.id}
          ...
          {assignment === "available" && (
            <button
              type="button"
              onClick={() => resources.assign(block.id)}
              disabled={resources.busyId === block.id}
  ...
  return (
    <div className="max-w-6xl space-y-6">
      <header className="flex flex-wrap items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-semibold">
            {agentId ? `Conocimiento de ${selectedAgent?.name}` : "Biblioteca de conocimiento"}
          </h2>
          <p className="text-sm text-[var(--text-muted)]">
            {agentId
              ? "Asigná únicamente los bloques que este agente debe recibir."
              : "Bloques reutilizables. Su contenido y estado son globales."}
          </p>
        </div>
        {canEdit && (
          <button
            ...
            className="ml-auto flex items-center gap-1.5 rounded bg-[var(--accent)] px-3 py-2 text-sm text-white"
          >
      ...
      {resources.loading ? (
        ...
        <>
          <section>
            <h3 className="mb-3 font-semibold">Asignados ({resources.assigned.length})</h3>
            <div className="grid gap-3 md:grid-cols-2">
              {resources.assigned.map((block) => (
                <BlockCard key={block.id} block={block} assignment="assigned" />
              ))}
              {resources.assigned.length === 0 && (
                <p className="text-sm text-[var(--text-muted)]">Sin bloques asignados.</p>
              )}
            </div>
          </section>
          <section>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {resources.available.map((block) => (
                <BlockCard key={block.id} block={block} assignment="available" />
              ))}
              {resources.available.length === 0 && (
                <p className="text-sm text-[var(--text-muted)]">No quedan bloques disponibles.</p>
              )}
            </div>
          </section>
        </>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {resources.library.map((block) => (
            <BlockCard key={block.id} block={block} />
          ))}
          {resources.library.length === 0 && (
            <p className="text-sm text-[var(--text-muted)]">No hay bloques en la biblioteca.</p>
          )}
        </div>
      )}
      {showCreate && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4">
          <section
            className="w-full max-w-lg rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)]"
            role="dialog"
            aria-modal="true"
            aria-labelledby="block-title"
          >
            <header className="flex items-center justify-between border-b border-[var(--border-color)] p-4">
              <h3 id="block-title" className="font-semibold">
                Nuevo bloque compartido
              </h3>
