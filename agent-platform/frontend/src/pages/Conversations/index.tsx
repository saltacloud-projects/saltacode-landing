...
export default function ConversationsPage() {
  ...
  const load = useCallback(async (agentId: string, selectedChannel: string) => {
    setError("");
    ...
    try {
      ...
      if (activeAgentId.current !== agentId) return;
      setError(
        value instanceof ApiError ? value.message : "No se pudieron cargar las conversaciones.",
      );
    }
  }, []);
  ...
  const open = async (conversation: Conversation) => {
    ...
    try {
      const rows = await api<Message[]>(
        `/conversations/${conversation.id}/messages?agent_id=${encodeURIComponent(agentId)}`,
      );
  ...
  const remove = async () => {
    const agentId = selectedAgent?.id;
    if (!agentId || !selected || !confirm("¿Eliminar esta conversación y todo su historial?"))
      return;
    setError("");
    try {
      await api(`/conversations/${selected.id}?agent_id=${encodeURIComponent(agentId)}`, {
        method: "DELETE",
      });
      if (activeAgentId.current !== agentId) return;
      setSelected(null);
      ...
      if (activeAgentId.current === agentId) {
        setError(
          value instanceof ApiError ? value.message : "No se pudo eliminar la conversación.",
        );
      }
    }
  };
  ...
  return (
    <div className="max-w-7xl">
      <header className="mb-5 flex flex-wrap items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-semibold">Conversaciones de {selectedAgent?.name}</h2>
          <p className="text-sm text-[var(--text-muted)]">
            Historial del agente seleccionado, separado por canal y ruta.
          </p>
        </div>
        <div className="ml-auto flex gap-2">
          ...
          <select
            ...
            className="rounded border border-[var(--border-color)] bg-[var(--bg-secondary)] px-3 py-2 text-sm"
            value={channel}
          ...
          <button
            ...
            onClick={() => {
              const agentId = selectedAgent?.id;
              if (agentId) void load(agentId, channel);
            }}
      ...
      <div className="grid min-h-[65vh] gap-4 lg:grid-cols-[minmax(280px,360px)_1fr]">
        <section className="overflow-hidden rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)]">
          {loading ? (
            <p className="flex items-center gap-2 p-4 text-sm text-[var(--text-muted)]">
            ...
            <div className="max-h-[70vh] divide-y divide-[var(--border-color)] overflow-y-auto">
              {items.map((conversation) => (
                <button
                  ...
                  className={`w-full p-4 text-left transition-colors hover:bg-[var(--bg-hover)] ${selected?.id === conversation.id ? "bg-[var(--bg-hover)]" : ""}`}
                >
                  <div className="flex items-center gap-2">
                    <strong className="truncate text-sm">
                      {conversation.display_name ||
                        `Visitante ${conversation.principal_id.slice(0, 8)}`}
                    </strong>
                    <span className="ml-auto rounded bg-[var(--accent)]/15 px-2 py-0.5 text-[10px] uppercase text-[var(--accent)]">
                      {conversation.channel}
                    </span>
                  </div>
                  <p className="mt-1 flex flex-wrap items-center gap-1 text-xs text-[var(--text-muted)]">
                    <code className="rounded bg-[var(--bg-secondary)] px-1 py-0.5">
        ...
        <section className="flex min-h-[420px] flex-col overflow-hidden rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)]">
          {!selected ? (
            <div className="grid flex-1 place-items-center p-6 text-center text-sm text-[var(--text-muted)]">
              Seleccioná una conversación para revisar su historial.
            </div>
          ) : (
            <>
              <header className="flex flex-wrap items-center gap-3 border-b border-[var(--border-color)] p-4">
                <div>
                  <h3 className="font-semibold">{selected.display_name || "Visitante"}</h3>
                  <p className="text-xs text-[var(--text-muted)]">
                    <span className="uppercase">{selected.channel}</span> · ruta{" "}
                    <code>{selected.route_key}</code> · consentimiento{" "}
                    {selected.transcript_consent
                      ? selected.consent_version || "registrado"
                      : "no registrado"}
                  </p>
                </div>
                {canDelete && (
                  <button
                    type="button"
                    onClick={() => void remove()}
                    className="ml-auto flex items-center gap-1 rounded border border-[var(--error)]/40 px-2 py-1.5 text-xs text-[var(--error)]"
                  >
              ...
              >
                {loadingMessages ? (
                  ...
                  messages.map((message) => (
                    <article
                      key={message.id}
                      className={`max-w-[88%] rounded-xl px-3 py-2 text-sm ${message.role === "user" ? "ml-auto bg-[var(--accent)] text-white" : "mr-auto border border-[var(--border-color)] bg-[var(--bg-secondary)]"}`}
                    >
                      ...
                      >
                        ...
                        {message.tool_names.map((tool) => (
                          <code key={tool} className="rounded bg-black/10 px-1">
                            {tool}
                          </code>
                        ))}
                      </footer>
