...
const INPUT =
  "w-full rounded-md border border-[var(--border-color)] bg-[var(--bg-secondary)] px-3 py-2 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--accent)]";
const PROMPTS: {
  key: "prompt_identity" | "prompt_domain" | "prompt_guardrails";
  label: string;
  help: string;
}[] = [
  { key: "prompt_identity", label: "Identidad", help: "Rol, tono y forma de presentarse." },
  {
    key: "prompt_domain",
    label: "Dominio",
    help: "Qué puede resolver y qué información debe priorizar.",
  },
  {
    key: "prompt_guardrails",
    label: "Guardrails",
    help: "Límites de seguridad, privacidad y comportamiento.",
  },
];
    ...
    <div className="max-w-6xl">
      <header className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)]">
            Elegí un agente para administrar su espacio de trabajo.
          </p>
        </div>
      </header>
      {error && (
      ...
      {loading ? (
        <p className="flex items-center gap-2 text-sm text-[var(--text-muted)]" role="status">
          <LoaderCircle className="animate-spin" size={16} /> Cargando agentes…
        </p>
      ) : (
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {profiles.map((profile) => (
            <article
              key={profile.id}
              className={`rounded-lg border bg-[var(--bg-card)] p-4 ${profile.is_active ? "border-[var(--success)]/60" : "border-[var(--border-color)]"}`}
            >
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="font-semibold">{profile.name}</h3>
                {profile.is_active && (
                  <span className="flex items-center gap-1 rounded bg-[var(--success)]/15 px-2 py-0.5 text-xs text-[var(--success)]">
              ...
              <p className="mt-3 text-xs text-[var(--text-muted)]">
                {profile.is_public ? "Disponible para web" : "Sin exposición web"} · historial{" "}
                {profile.retention_days} días
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
                <Link
                  to={`/agents/${profile.id}/overview`}
                  className="flex items-center gap-1 rounded bg-[var(--accent)] px-3 py-1.5 text-sm text-white"
                >
                  <ExternalLink size={14} /> Abrir espacio
                </Link>
                {canEdit && !profile.is_active && (
                  <button
                    ...
                    className="flex items-center gap-1 rounded border border-[var(--border-color)] px-3 py-1.5 text-sm disabled:opacity-50"
                  >
          ...
          {profiles.length === 0 && (
            <p className="text-sm text-[var(--text-muted)]">No hay agentes disponibles.</p>
          )}
        </div>
      )}
    </div>
    ...
    <div className="max-w-4xl space-y-4">
      <header className="flex flex-wrap items-center gap-3">
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)]">
            Perfil, comportamiento, disponibilidad y retención del agente seleccionado.
          </p>
        </div>
        {canEdit && (
          <button
      ...
      <section className="grid gap-4 rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-5 md:grid-cols-2">
        ...
        <label className="flex items-center gap-2 self-end rounded border border-[var(--border-color)] p-3 text-sm">
          <input
            disabled={!canEdit}
            type="checkbox"
          ...
          />{" "}
          Disponible para selección web pública
        </label>
        ...
        >
          <div className="mb-2 flex flex-wrap items-baseline justify-between gap-2">
            ...
            <span className="text-xs text-[var(--text-muted)]">
              {draft[field.key].length.toLocaleString("es-AR")} caracteres
            </span>
          </div>
          <textarea
            id={field.key}
      ...
      <section className="grid gap-4 rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-5 md:grid-cols-2">
        <label className="text-sm">
          ...
          <textarea
            ...
            rows={3}
            className={`${INPUT} mt-1`}
            value={draft.unauthorized_message}
            onChange={(event) => setDraft({ ...draft, unauthorized_message: event.target.value })}
          />
        </label>
        <label className="text-sm">
          Mensaje ante error
          <textarea
            ...
            onChange={(event) => setDraft({ ...draft, error_message: event.target.value })}
          />
        </label>
      </section>
    </div>
