...
The agent platform is an independent release unit. Its API, panel, optional RAG worker, migrations, bootstrap, PostgreSQL, Redis, and document store are operated from `agent-platform/`; the landing release scripts never manage them.
...
It checks Docker/Compose capabilities, the clean production checkout, immutable release naming, loopback bindings, protected files, non-placeholder credentials, build inputs, effective image tags, and the rendered Compose model.
...
5. Record the current Alembic revision and stop only API, panel, and optional RAG worker.
6. Run `alembic upgrade head` through the one-shot `migrate` service, then run bootstrap.
7. Start API and panel with `--no-deps`; start the worker only when configured.
8. Verify API readiness inside the container and through loopback, plus panel HTTP inside the container and through loopback.
...
Rollback audit receipts live under `rollbacks/`. They contain release identifiers, Git commit, contract hashes, database revisions, immutable image IDs, RAG state, verification state, and timestamps. They never contain environment values, passwords, provider keys, internal tokens, document contents, or conversation data. See [`release-receipt.example`](release-receipt.example).
...
Rollback verifies both releases before stopping anything, never runs migrations or bootstrap, starts only the target API/panel/optional worker images, verifies them, records the operation, and updates the pointer. On a failed target start it attempts to restore the release that was current when rollback began.
