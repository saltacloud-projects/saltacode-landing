...
async def _get_or_create_area(db, name: str, actor_id: uuid.UUID) -> OrganizationArea:
    area = (
        await db.execute(select(OrganizationArea).where(OrganizationArea.name == name))
    ).scalar_one_or_none()
    if area is not None:
        return area
...
async def import_corpus(root: Path) -> None:
    ...
    supported = [
        path for path in files if path.suffix.lower().lstrip(".") in ALLOWED_EXTENSIONS
    ]
    ...
    async with AsyncSessionLocal() as db:
        ...
        if admin is None:
            raise SystemExit(
                "No existe un administrador activo para auditar la importación"
            )
        admin_id = admin.id
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                if admin is None or root_folder is None:
                    raise RuntimeError(
                        "El contexto persistido de la importación ya no existe"
                    )
                ...
                if existing is not None:
                    ...
                    if not document_storage.exists(blob.storage_key):
                        ...
                        if stored.sha256 != blob.sha256:
                            ...
                            raise RuntimeError(
                                "El archivo fuente cambió desde la importación inicial"
                            )
                        restored += 1
                    skipped += 1
                    continue
                ...
                with path.open("rb") as source:
                    upload = UploadFile(
                        file=source, filename=path.name, size=path.stat().st_size
                    )
            ...
            rejected += 1
            reason = (
                str(exc)[:160] if isinstance(exc, StorageError) else type(exc).__name__
            )
            print(
                f"rejected extension={path.suffix.lower() or '[none]'} reason={reason}"
            )
