...
def require(response: httpx.Response, expected: int | tuple[int, ...] = 200):
    ...
    if response.status_code not in expected_values:
        ...
        raise RuntimeError(
            f"{response.request.method} {response.request.url.path}: {response.status_code} {detail}"
        )
...
def wait_for_version(
    client: httpx.Client, document_id: str, minimum_version: int, timeout: int = 240
):
    ...
    while time.monotonic() < deadline:
        ...
        if (
            current
            and current["version_number"] >= minimum_version
        ...
        if job.get("status") == "failed":
            raise RuntimeError(
                f"La ingesta falló: {job.get('error_code')} {job.get('error_message')}"
            )
        time.sleep(2)
    raise RuntimeError(
        f"Timeout esperando versión {minimum_version}; último estado={last}"
    )
...
async def provision_temporary_document_manager() -> tuple[str, str, uuid.UUID]:
    ...
    async with AsyncSessionLocal() as db:
        ...
        legacy_user = next(
            (item for item in users if item.email == LEGACY_TEMPORARY_EMAIL), None
        )
        if user is None:
            user = legacy_user
...
def main() -> None:
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Confirma que se crearán registros QA en el entorno indicado",
    )
    parser.add_argument(
        "--base-url",
        default=os.getenv(
            "PANEL_E2E_BASE_URL", "https://panelagente.saltacloud.com/api/admin"
        ),
    )
    parser.add_argument(
        "--temporary-user",
    ...
    try:
        with httpx.Client(
            ...
            if not expected_permissions.issubset(set(me.get("permissions") or [])):
                raise RuntimeError(
                    "La cuenta de prueba no tiene los permisos documentales requeridos"
                )

            # El rol debe quedar aislado del resto del panel y de la configuración RAG.
            require(client.get("/tools/"), 403)
            require(
                client.patch("/documents/settings", json={"retention_days": 30}), 403
            )
            require(
                client.post(
                    "/documents/areas",
                    json={"name": f"QA {run_id}", "slug": f"qa-{run_id}"},
                ),
                403,
            )
            ...
            area_id = next(
                (area["id"] for area in active if area["is_general"]), active[0]["id"]
            )
            ...
            multiple = upload(
                ...
                files=[
                    (
                        f"qa-multiple-a-{run_id}.md",
                        f"# QA A\nContenido múltiple A {run_id}.".encode(),
                        "text/markdown",
                        f"QA-{run_id}/Subcarpeta-A",
                    ),
                    (
                        f"qa-multiple-b-{run_id}.txt",
                        f"Contenido múltiple B {run_id}.".encode(),
                        "text/plain",
                        f"QA-{run_id}/Subcarpeta-B",
                    ),
                ],
            )
            ...
            with zipfile.ZipFile(archive_bytes, "w", zipfile.ZIP_DEFLATED) as archive:
                archive.writestr(
                    f"QA-{run_id}/zip-a.txt",
                    f"Contenido ZIP A {run_id} suficientemente descriptivo.",
                )
                archive.writestr(
                    f"QA-{run_id}/zip-b.md",
            ...
            updated = require(
                client.patch(
                    ...
                    json={
                        "title": f"QA funcional {run_id}",
                        "description": "Documento temporal de validación",
                    },
                )
            )
            ...
            replacement_content = (
                f"Reemplazo QA {run_id} con contenido diferente y recuperable.".encode()
            )
            replacement = require(
                client.post(
            ...
            duplicate_replacement = require(
                client.post(
                    f"/documents/{first['document_id']}/replace",
                    files={
                        "file": (
                            f"qa-reemplazo-duplicado-{run_id}.txt",
                            replacement_content,
                            "text/plain",
                        )
                    },
            ...
            if not duplicate_replacement["accepted"][0]["duplicate_hash"]:
                raise RuntimeError(
                    "El reemplazo duplicado no fue identificado por hash"
                )
            wait_for_version(client, first["document_id"], 3)

            versions_before = require(
                client.get(f"/documents/{first['document_id']}/versions")
            )
            reindex = require(
                client.post(f"/documents/{first['document_id']}/reindex"), 202
            )
            if reindex["status"] != "queued":
                raise RuntimeError(f"Reindexación no encolada: {reindex}")
            wait_for_version(
                client, first["document_id"], versions_before[0]["version_number"] + 1
            )
            ...
            restored = require(
                client.post(f"/documents/{first['document_id']}/restore")
            )
        ...
        if created_ids:
            try:
                with httpx.Client(
                    ...
                    cleanup_client.headers["Authorization"] = (
                    ...
                    for document_id in created_ids:
                        ...
                        if response.status_code not in {204, 404}:
                            print(
                                f"WARN cleanup document_id={document_id} status={response.status_code}"
                            )
            except (
                Exception
            ) as exc:  # La falla original conserva prioridad sobre la limpieza.
