...
test("dashboard separates operations, automation, agent settings, and global libraries", async ({
  ...
  await expect(content.getByRole("link", { name: /^Reuniones/ })).toHaveAttribute(
    "href",
  ...
  await expect(content.getByRole("link", { name: /^Ingresos externos/ })).toHaveAttribute(
    "href",
    `/agents/${AGENT_A}/inbound-jobs`,
  );
