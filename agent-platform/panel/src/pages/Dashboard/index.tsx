...
const LINKS: WorkspaceLink[] = [
  {
    label: "Identidad",
    description: "Prompts, mensajes, exposición y retención.",
    segment: "identity",
    icon: Settings2,
  ...
  {
    label: "Conocimiento",
    description: "Bloques asignados desde la biblioteca.",
    segment: "knowledge",
    icon: Brain,
  ...
  {
    label: "Documentos",
    description: "Áreas documentales asociadas al agente.",
    segment: "documents",
    icon: Files,
  ...
  {
    label: "Fuentes",
    description: "APIs y secretos write-only asignados.",
    segment: "sources",
    icon: Cable,
  ...
  {
    label: "Herramientas",
    description: "Capacidades HTTP habilitadas para el agente.",
    segment: "tools",
    icon: Wrench,
  ...
  {
    label: "Runtime",
    description: "Proveedor, modelos, límites, memoria y RAG.",
    segment: "runtime",
    icon: Cpu,
  ...
  {
    label: "Canales",
    description: "Rutas web y WhatsApp resueltas por el servidor.",
    segment: "channels",
    icon: Globe2,
    permission: PERMISSIONS.RUNTIME_READ,
  },
  {
    ...
    description: "Identidades y áreas autorizadas para este agente.",
    segment: "access",
    icon: ShieldCheck,
  ...
  },
  {
    ...
    description: "Historial del agente seleccionado.",
  ...
  {
    label: "PromptLab",
    description: "Previsualización y pruebas controladas.",
    segment: "promptlab",
    icon: FlaskConical,
  ...
  return (
    <div className="max-w-6xl space-y-6">
        ...
        <article className="rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-4">
          ...
          <p className="mt-2 flex items-center gap-2 font-medium">
            {selectedAgent.is_active ? (
              ...
              <CircleOff className="text-amber-400" size={17} />
            )}
            {selectedAgent.is_active ? "Activo" : "Inactivo"}
          </p>
        </article>
        <article className="rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-4">
          <p className="text-xs text-[var(--text-muted)]">Web pública</p>
          <p className="mt-2 font-medium">
            {selectedAgent.is_public ? "Disponible" : "No disponible"}
          </p>
        </article>
        <article className="rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-4">
          ...
          {visibleLinks.map((item) => {
            ...
            return (
              <Link
                ...
                className="group rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-4 transition-colors hover:border-[var(--accent)]"
              >
      ...
      <aside className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 text-sm text-amber-200">
        Las bibliotecas de fuentes, herramientas, conocimiento y documentos son compartidas. La
        asignación determina qué recursos pertenecen a este agente; editar un recurso de biblioteca
        puede afectar a otros agentes que también lo usen.
      </aside>
    </div>
  );
}
