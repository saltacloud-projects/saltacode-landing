...
export default function AgentAccessPage() {
    ...
    async (agentId: string) => {
      ...
      try {
        const [libraryRows, assignedRows, areaRows] = await Promise.all([
          ...
          canReadAreas
            ? api<AgentDocumentArea[]>(`/agents/${agentId}/document-areas`)
            : Promise.resolve([]),
        ]);
  ...
  const openIdentity = (identity: GlobalAuthorizedIdentity) => {
    ...
    setIdentityDraft({
      mode: "edit",
      id: identity.id,
      originalPhone: identity.phone_number,
    ...
    try {
      if (identityDraft.mode === "create") {
        const created = await api<GlobalAuthorizedIdentity>("/users/", {
          method: "POST",
          body: JSON.stringify({
            phone_number: identityDraft.phone.trim(),
            name: identityDraft.name.trim() || null,
            notes: identityDraft.notes.trim() || null,
          }),
        });
        ...
        if (activeAgentId.current === agentId)
          setNotice(
            identityDraft.assignAfterCreate
              ? "Identidad creada y asignada al agente con una política inicial."
              : "Identidad creada en la biblioteca global.",
          );
      } else {
        ...
        if (activeAgentId.current === agentId)
          setNotice("Identidad global actualizada. La política del agente no fue modificada.");
      }
      if (activeAgentId.current !== agentId) return;
      setIdentityDraft(null);
      await load(agentId);
      ...
      if (activeAgentId.current === agentId)
        setError(value instanceof Error ? value.message : "No se pudo guardar la identidad.");
    } finally {
  ...
  const savePolicy = async () => {
    ...
    setNotice("");
    try {
      await api(`/agents/${agentId}/authorized-users/${policyDraft.userId}`, {
        method: "PUT",
        body: JSON.stringify(assignmentBody(policyDraft)),
      });
      if (activeAgentId.current !== agentId) return;
      ...
      if (activeAgentId.current === agentId)
        setError(value instanceof Error ? value.message : "No se pudo guardar la política.");
    } finally {
      if (activeAgentId.current === agentId) setSaving(false);
    }
  };
  ...
  const unassign = async (identity: AgentAuthorizedUser) => {
    if (
      !window.confirm(
        `¿Quitar el acceso de ${identity.name || identity.phone_number} a ${selectedAgent.name}?`,
      )
    )
      return;
    const agentId = selectedAgent.id;
    ...
    try {
      ...
      if (activeAgentId.current === agentId)
        setError(value instanceof Error ? value.message : "No se pudo quitar el acceso.");
    } finally {
  ...
  return (
    <div className="max-w-6xl space-y-5">
      <header className="flex flex-wrap items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-semibold">Acceso WhatsApp de {selectedAgent.name}</h2>
          <p className="text-sm text-[var(--text-muted)]">
            Identidades reutilizables con una política independiente para este agente.
          </p>
        </div>
        {canManage && !identityDraft && !policyDraft && (
          <button
            type="button"
            onClick={openCreate}
            className="ml-auto flex items-center gap-2 rounded bg-[var(--accent)] px-3 py-2 text-sm text-white"
          >
            <Plus size={16} /> Nueva identidad
          </button>
        )}
      </header>
      <aside className="rounded border border-[var(--border-color)] bg-[var(--bg-card)] p-4 text-sm text-[var(--text-secondary)]">
        Teléfono, nombre y notas pertenecen a la identidad global. Estado y áreas documentales se
        guardan únicamente en el vínculo con{" "}
        <strong className="text-[var(--text-primary)]">{selectedAgent.name}</strong>.
      </aside>
      {error && (
        <p
      ...
      {identityDraft && (
        <section
          className="space-y-4 rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] p-5"
        ...
        >
          <div className="flex items-center gap-3">
            <button
              ...
              aria-label="Cerrar editor de identidad"
              className="rounded p-1 hover:bg-[var(--bg-hover)]"
            >
              <ArrowLeft size={17} />
            </button>
            <div>
              <h3 id="identity-editor-title" className="font-semibold">
                {identityDraft.mode === "create"
                  ? "Nueva identidad global"
                  : "Editar identidad global"}
              </h3>
              <p className="text-xs text-[var(--text-muted)]">
                Este formulario no modifica la política de otros agentes.
              </p>
            </div>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <label className="text-xs">
              Teléfono
              <input
                disabled={identityDraft.mode === "edit"}
                className={`${INPUT} mt-1 font-mono`}
                value={identityDraft.phone}
                onChange={(event) =>
                  setIdentityDraft({ ...identityDraft, phone: event.target.value })
                }
            ...
            <label className="text-xs">
              ...
              <input
                ...
                onChange={(event) =>
                  setIdentityDraft({ ...identityDraft, name: event.target.value })
                }
              />
            </label>
            <label className="text-xs md:col-span-2">
              ...
              <textarea
                ...
                onChange={(event) =>
                  setIdentityDraft({ ...identityDraft, notes: event.target.value })
                }
              />
            </label>
            {identityDraft.mode === "create" && (
              <label className="flex items-center gap-2 text-sm md:col-span-2">
                <input
                  type="checkbox"
                  checked={identityDraft.assignAfterCreate}
                  onChange={(event) =>
                    setIdentityDraft({ ...identityDraft, assignAfterCreate: event.target.checked })
                  }
                />{" "}
                Asignar ahora a {selectedAgent.name} con una política inicial
              </label>
            )}
          </div>
        ...
        >
          <div className="flex items-center gap-3">
            <button
              ...
              aria-label="Cerrar editor de acceso"
              className="rounded p-1 hover:bg-[var(--bg-hover)]"
            >
              <ArrowLeft size={17} />
            </button>
            <div>
              <h3 id="policy-editor-title" className="font-semibold">
                Acceso de {policyDraft.name}
              </h3>
              <p className="text-xs text-[var(--text-muted)]">
                Política exclusiva de {selectedAgent.name}. El área General se agrega
                automáticamente cuando está asignada al agente.
              </p>
            </div>
          </div>
          <div className="space-y-3">
            <label className="flex items-center gap-2 text-sm">
              <input
                ...
                onChange={(event) =>
                  setPolicyDraft({ ...policyDraft, isActive: event.target.checked })
                }
              />{" "}
              Acceso activo
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={policyDraft.hasAllAreaAccess}
                onChange={(event) =>
                  setPolicyDraft({ ...policyDraft, hasAllAreaAccess: event.target.checked })
                }
              />{" "}
              Acceso a todas las áreas asignadas al agente
            </label>
            <fieldset
              disabled={policyDraft.hasAllAreaAccess || !canReadAreas}
              className="grid gap-2 rounded border border-[var(--border-color)] p-3 sm:grid-cols-2 disabled:opacity-60"
            >
              <legend className="px-1 text-xs font-medium">Áreas adicionales</legend>
              {selectableAreas.map((area) => (
                <label key={area.id} className="flex items-center gap-2 text-sm">
                  <input
                    type="checkbox"
                    checked={policyDraft.areaIds.includes(area.id)}
                    onChange={(event) =>
                      setPolicyDraft({
                        ...policyDraft,
                        areaIds: event.target.checked
                          ? [...policyDraft.areaIds, area.id]
                          : policyDraft.areaIds.filter((id) => id !== area.id),
                      })
                    }
                  />{" "}
                  {area.name}
              ...
              {selectableAreas.length === 0 && (
                <p className="text-xs text-[var(--text-muted)] sm:col-span-2">
                  No hay áreas adicionales asignadas a este agente.
                </p>
              )}
            </fieldset>
            {!canReadAreas && (
              <p className="text-xs text-amber-300">
                No tenés permiso para leer las áreas documentales; no podés modificar ese alcance.
              </p>
            )}
          </div>
          <div className="flex justify-end gap-2">
      ...
      {loading ? (
        ...
        <>
          <section>
            <h3 className="mb-3 font-semibold">Con acceso ({assigned.length})</h3>
            <div className="grid gap-3 lg:grid-cols-2">
              {assigned.map((identity) => (
                <article
                  key={identity.id}
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className="min-w-0 flex-1">
                      <h4 className="font-semibold">{identity.name || "Sin nombre"}</h4>
                      <code className="mt-1 block text-xs text-[var(--text-muted)]">
                        {identity.phone_number}
                      </code>
                    </div>
                    <span
                      className={`rounded px-2 py-1 text-xs ${identity.is_active ? "bg-emerald-500/10 text-emerald-400" : "bg-amber-500/10 text-amber-300"}`}
                    >
                      {identity.is_active ? "Activo" : "Pausado"}
                    </span>
                  </div>
                  <p className="mt-3 text-sm text-[var(--text-secondary)]">
                    {areaSummary(identity)}
                  </p>
                  {identity.notes && (
                    <p className="mt-2 text-xs text-[var(--text-muted)]">{identity.notes}</p>
                  )}
                  {canManage && (
                    <div className="mt-4 flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() => openPolicy(identity)}
                        className="flex items-center gap-1 rounded bg-[var(--accent)] px-2 py-1.5 text-xs text-white"
                      >
                        <ShieldCheck size={14} /> Editar acceso
                      </button>
                      <button
                        type="button"
                        onClick={() => openIdentity(identity)}
                        className="flex items-center gap-1 rounded border border-[var(--border-color)] px-2 py-1.5 text-xs"
                      >
                        <Pencil size={14} /> Editar identidad
                      </button>
                      <button
                        type="button"
                        onClick={() => unassign(identity)}
                      ...
                      >
                        <UserMinus size={14} /> Quitar acceso
                      </button>
                    </div>
                  )}
                </article>
              ...
              {assigned.length === 0 && (
                <p className="text-sm text-[var(--text-muted)]">
                  Este agente todavía no tiene identidades autorizadas.
                </p>
              )}
            </div>
          </section>
          <section>
            <h3 className="mb-3 font-semibold">Disponibles para reutilizar ({available.length})</h3>
            <div className="grid gap-3 lg:grid-cols-2">
              ...
              {available.length === 0 && (
                <p className="text-sm text-[var(--text-muted)]">
                  No quedan identidades globales disponibles.
                </p>
              )}
            </div>
          </section>
