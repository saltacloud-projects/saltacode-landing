...
The optimized animated brand lockup adds 1,141 bytes of raw home HTML and 331 bytes of emitted CSS over the post-theme build, adds no JavaScript or font request, and removes the eager raster hero mark. The current build is 27,042 bytes of HTML, 18,297 bytes of CSS, 3,953 bytes of initial JavaScript, and 12,253 bytes of total JavaScript, leaving 606, 135, 1,167, and 2,083 bytes of headroom respectively without raising a limit.

The equivalent post-lockup mobile lab used the same conditions and three cold runs per explicit theme. Light mode recorded 113,845 bytes in every run, 572/576/588 ms LCP, 25/30/55 ms TBT, and 0 CLS; dark mode recorded 111,481 bytes in every run, 564/572/584 ms LCP, 29/30/31 ms TBT, and 0 CLS. No run fetched the wrong theme variant, overflowed horizontally, or raised a runtime exception. This is local lab evidence only; production compression, field data, and Lighthouse remain release-stage checks.
